...
```
ssh cccna@69.196.164.58 -i ~/.ssh/ccc2016mtl_rsa -L 8080:cs1.cloud.lan:8080
```
...
```
[cccna@bubble-ci-3 ~]$ ssh cs1
```

if you need to SSH to KVM servers:
```
[cccna@bubble-ci-3 ~]$ ssh kvm1
[cccna@bubble-ci-3 ~]$ ssh kvm2
```
...
```
[cccna@bubble-ci-3 ~]$ cd /data/shared/deploy
[cccna@bubble-ci-3 ~]$ ./kvm_local_deploy.py -x kvm1 && ./kvm_local_deploy.py -x kvm2
[cccna@bubble-ci-3 ~]$ cd /data/shared/helper_scripts/cleaning
[cccna@bubble-ci-3 ~]$ sudo ./clean_storage.sh
[cccna@bubble-ci-3 ~]$ ssh cs1
